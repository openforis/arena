...
class ApplicableIfUpdater {
  ...
  async updateNodesApplicability (user, survey, nodesArray, tx) {
    return R.mergeAll(
      await Promise.all(
        nodesArray.map(async node => await this.updateNodeApplicability(survey, node, tx))
      )
    )
  }

  async updateNodeApplicability (survey, node, tx) {
