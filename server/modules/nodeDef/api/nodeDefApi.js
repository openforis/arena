...
export const init = (app) => {
  ...
  app.put(
    ...
    async (req, res, next) => {
      try {
        ...
        const { dependencyGraph, nodeDefsUpdated, nodeDefsValidation } = await NodeDefService.updateNodeDefProps({
          user,
          surveyId,
          cycle,
          nodeDefUuid,
        ...
        res.json({ dependencyGraph, nodeDefsUpdated, nodeDefsValidation })
      } catch (error) {
        next(error)
      }
    }
  ...
  app.post(
    '/survey/:surveyId/nodeDef/clone-from-survey',
    AuthMiddleware.requireSurveyEditPermission,
    async (req, res, next) => {
      try {
        const { surveyId } = Request.getParams(req)
        ...
        const { nodeDefsUpdated, nodeDefsValidation } = await NodeDefService.cloneNodeDefFromSurvey({
          sourceSurveyId,
          sourceNodeDefUuid,
