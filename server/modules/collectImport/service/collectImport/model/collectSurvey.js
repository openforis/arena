...
export const samplingPointDataCodeListNames = [
  'sampling_design',
  'ofc_sampling_design',
]
...
export const toLabels = (
  ...
  R.pipe(
    ...
    R.reduce((labelsAcc, labelEl) => {
      ...
      if (
        !R.has(lang, labelsAcc) &&
        (R.isEmpty(typesFilter) || R.includes(type, typesFilter))
      ) {
...
export const getChildElementText = name =>
  R.pipe(getElementByName(name), getText)

export const getAttributes = R.propOr({}, keys.attributes)

export const getAttribute = (name, defaultValue = null) =>
  R.pipe(getAttributes, R.propOr(defaultValue, name))
...
export const getAttributeBoolean = name =>
  R.pipe(getAttribute(name), R.equals('true'))
...
export const getNodeDefChildren = collectNodeDef =>
  R.pipe(getElements, R.filter(_isNodeDefElement))(collectNodeDef)

const _isNodeDefElement = R.pipe(getElementName, name =>
  R.includes(name, R.keys(collectNodeDefTypes)),
)
