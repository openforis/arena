...
const _validateCategoryFromCategories = async (
  ...
  const items = await CategoryRepository.fetchItemsByCategoryUuid(
    surveyId,
    categoryUuid,
    true,
    client,
  ...
  await CategoryRepository.updateCategoryValidation(
    surveyId,
    categoryUuid,
    validation,
    client,
...
export const validateCategory = async (surveyId, categoryUuid, client = db) => {
  ...
  return await _validateCategoryFromCategories(
    surveyId,
    categories,
    categoryUuid,
    client,
...
export const validateCategories = async (surveyId, client = db) => {
  ...
  const categoriesValidated = await Promise.all(
    Object.keys(categories).map(categoryUuid =>
      _validateCategoryFromCategories(
        surveyId,
        categories,
        categoryUuid,
        client,
  ...
  await client.tx(async t => {
    const [categoryDb] = await Promise.all([
      ...
      ...Category.getLevelsArray(category).map(level =>
        insertLevel(user, surveyId, level, true, t),
      ),
  ...
  await client.tx(async t => {
    ...
    return {
      ...
      category: await validateCategory(
        surveyId,
        CategoryLevel.getCategoryUuid(level),
        t,
      ),
    }
  })
...
export const insertItems = async (user, surveyId, items, client = db) =>
  await client.tx(async t => {
    const activityLogs = items.map(item =>
      ActivityLog.newActivity(ActivityLog.type.categoryItemInsert, item, true),
    )
    await Promise.all([
...
export const createImportSummary =
  CategoryImportSummaryGenerator.createImportSummary
export const createImportSummaryFromStream =
  CategoryImportSummaryGenerator.createImportSummaryFromStream

// ====== READ
export const fetchCategoriesAndLevelsBySurveyId =
  CategoryRepository.fetchCategoriesAndLevelsBySurveyId
export const fetchCategoryAndLevelsByUuid =
  CategoryRepository.fetchCategoryAndLevelsByUuid
export const fetchItemsByCategoryUuid =
  CategoryRepository.fetchItemsByCategoryUuid
  ...
  await client.tx(
    async t =>
      await Promise.all([
        ...
        ...langsDeleted.map(langDeleted =>
          CategoryRepository.deleteItemLabels(surveyId, langDeleted, t),
        ),
      ]),
  )

export const updateCategoryProp = async (
  user,
  surveyId,
  categoryUuid,
  key,
  ...
  await client.tx(async t => {
    await Promise.all([
      CategoryRepository.updateCategoryProp(
        surveyId,
        categoryUuid,
        key,
        value,
...
export const updateLevelProp = async (
  user,
  surveyId,
  categoryUuid,
  levelUuid,
...
const _newCategoryItemUpdateLogActivity = (
  categoryUuid,
  item,
  key,
  value,
...
export const updateItemProp = async (
  user,
  surveyId,
  categoryUuid,
  itemUuid,
  ...
  await client.tx(async t => {
    const item = await CategoryRepository.updateItemProp(
      surveyId,
      itemUuid,
      key,
      value,
...
export const updateItemsExtra = async (
  user,
  surveyId,
  categoryUuid,
  items,
...
export const deleteLevel = async (
  ...
  await client.tx(async t => {
    const levelDeleted = await CategoryRepository.deleteLevel(
      surveyId,
      levelUuid,
      t,
    )
 ...
 */
export const deleteLevelsEmptyByCategory = async (
  user,
  surveyId,
  category,
  client = db,
  ...
  await client.tx(async t => {
    ...
    const levelsUpdated = R.reject(level =>
      R.includes(CategoryLevel.getUuid(level), levelUuidsDeleted),
    )(levels)
 ...
 * Category validation is not performed
 */
export const replaceLevels = async (
  user,
  surveyId,
  category,
  levelNamesNew,
  ...
  await client.tx(async t => {
    ...
    await Promise.all([
      ...
      ActivityLogRepository.insert(
        user,
        surveyId,
        ActivityLog.type.categoryLevelsDelete,
        logContent,
        true,
        t,
      ),
