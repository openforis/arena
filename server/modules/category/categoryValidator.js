...
const levelValidators = (levels, itemsByParentUuid) => ({
  [`${CategoryLevel.keys.props}.${CategoryLevel.keysProps.name}`]: [
    ...
    Validator.validateItemPropUniqueness(
      Validation.messageKeys.categoryEdit.levelDuplicate,
    )(levels),
  ],
...
const validateItemCodeUniqueness = itemsByCode => (propName, item) => {
...
const itemValidators = (isLeaf, itemsByParentUuid, siblingsByCode) => ({
  [`${CategoryItem.keys.props}.${CategoryItem.props.code}`]: [
    Validator.validateRequired(
      Validation.messageKeys.categoryEdit.codeRequired,
    ),
    Validator.validateNotKeyword(
      Validation.messageKeys.categoryEdit.codeCannotBeKeyword,
    ),
...
const validateItem = (
  ...
  const validation = await Validator.validate(
    item,
    itemValidators(isLeaf, itemsByParentUuid, siblingsByCode),
  )
  ...
  const childValidations = await validateItemsByParentUuid(
    category,
    itemsByParentUuid,
    itemUuid,
  )

  const combinedValidation = Validation.isValid(childValidations)
    ? validation
    : R.pipe(
  ...
  return R.pipe(Validation.getFieldValidations, fieldValidations => ({
    ...fieldValidations,
    [itemUuid]: combinedValidation,
  }))(childValidations)
}

const validateItemsByParentUuid = async (
  category,
  itemsByParentUuid,
  parentItemUuid,
) => {
  ...
  const childrenValidationsMerged = childrenValidationsArr.reduce(
    (validationsAcc, childValidations) => {
...
export const validateCategory = async (categories, category, items) => {
  const itemsByParentUuid = R.groupBy(R.prop(CategoryItem.keys.parentUuid))(
    items,
  )
  ...
  const itemsValidation = await validateItemsByParentUuid(
    category,
    itemsByParentUuid,
    null,
  )
