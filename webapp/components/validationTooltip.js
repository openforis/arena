...
const ValidationTooltip = (props) => {
  const { validation, className, showKeys, children, id, isCell } = props
  ...
  return (
    <Tooltip type={type} messageComponent={content} className={className} showContent={showContent} id={id} isCell={isCell}>
      {children}
    </Tooltip>
  )
}
