 ...
 * ============
 */
export const checkInRecord = (recordUuid, draft, focusNodeUuid, focusNodeDefUuid) => async (dispatch, getState) => {
  dispatch(showAppLoader())
  ...
  if (focusNodeUuid) {
    ...
    const entity = Record.getNodeByUuid(focusNodeUuid)(record)
    const ancestors = Record.getAncestorsAndSelf(entity)(record)
    const focusNodeDef = Survey.getNodeDefByUuid(focusNodeDefUuid)(survey)
    ...
    */
    const nodeDefActivePage =
      focusNodeDef && NodeDefLayout.hasPage(cycle)(focusNodeDef)
        ? focusNodeDef
        : R.pipe(
