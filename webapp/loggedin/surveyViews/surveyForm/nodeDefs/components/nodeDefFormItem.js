...
const NodeDefFormItemLabel = props => {
  ...
  return (
    <NodeDefErrorBadge nodeDef={nodeDef} edit={edit} parentNode={parentNode} nodes={nodes}>
      <div>
        {NodeDef.isKey(nodeDef) && <span className="icon icon-key2 icon-10px icon-left node-def__icon-key" />}
        {label}
      </div>
    </NodeDefErrorBadge>
  )
...
const NodeDefFormItem = props => {
  ...
  return NodeDef.isEntity(nodeDef) ? (
    ...
    <FormItem label={<NodeDefFormItemLabel {...props} />} className="survey-form__node-def-form-item">
