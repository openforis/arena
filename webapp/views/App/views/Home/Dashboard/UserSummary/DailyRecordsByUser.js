...
//   if (!userDateCounts) return {}
//   return userDateCounts.reduce((acc, item) => {
//     const { date, owner_name: name, owner_email: email } = item
//     let nameOrEmail = name ?? email.substring(0, email.indexOf('@'))
//     if (!acc[nameOrEmail]) {
//       acc[nameOrEmail] = 
//     }
//     const dateIndex = Math.floor((lastDate - new Date(item.date)) / dayInMs)
//     if (dateIndex < daysDiff) {
//       acc[nameOrEmail][dateIndex] = parseInt(item.count)
//     }
//     return acc
//   }, {})
// }
...
const DailyRecordsByUser = () => {
    ...
    () =>
      Object.values(
        userDateCounts.reduce((acc, item) => {
          const { count, date, owner_uuid } = item
          let obj = acc[date]
          if (!obj) {
            obj = { date }
            acc[date] = obj
          }
          obj[owner_uuid] = Number(count)
          return acc
        }, {})
      ),
    [userDateCounts]
  ...
  const chartDataKeys = useMemo(() => selectedUsers.map((item) => item.owner_uuid), [selectedUsers])
  ...
  return (
    <>
      ...
      {chartData.length > 0 && chartDataKeys.length > 0 && (
        <AreaChart data={chartData} dataKeys={chartDataKeys} labelDataKey="date" />
      )}
