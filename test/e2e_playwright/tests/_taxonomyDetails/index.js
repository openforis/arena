...
export const editTaxonomyDetails = (taxonomy) => {
  test(`Edit taxonomy ${taxonomy.name} details`, async () => {
    await page.fill(getSelector(DataTestId.taxonomyDetails.taxonomyName, 'input'), taxonomy.name)
    await page.fill(getSelector(DataTestId.taxonomyDetails.taxonomyDescription(), 'input'), taxonomy.description)
  })
