...
export const verifyNodeDefs = (survey) => {
  test(`Verify ${survey.name} nodeDefs`, async () => {
    ...
    for (const childDef of clusterExportChildDefs) {
      await verifyNodeDef(childDef, cluster.children[_getNodeDefName(childDef)])
    }
    ...
    await expect(getProps(countryDef).parentCodeDefUuid).toBeNull()
    ...
    for (const childDef of plotExportChildDefs) {
      await verifyNodeDef(childDef, plot.children[_getNodeDefName(childDef)])
    }
    ...
    for (const childDef of treeExportChildDefs) {
      await verifyNodeDef(childDef, tree.children[_getNodeDefName(childDef)])
    }
  })
}
