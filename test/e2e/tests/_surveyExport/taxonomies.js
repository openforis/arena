...
export const verifyTaxonomies = (survey) =>
  test('Verify taxonomies', async () => {
    const taxonomiesExport = Object.values(getSurveyEntry(survey, 'taxonomies', 'taxonomies.json'))
    ...
    await PromiseUtils.each(taxonomiesExport, async (taxonomyExport) => {
      ...
      const taxaExport = getSurveyEntry(survey, 'taxonomies', `${taxonomyExport.uuid}.json`)
