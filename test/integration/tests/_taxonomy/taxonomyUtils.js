...
export const getTaxonVernacularNamesText = (lang) =>
  A.pipe(Taxon.getVernacularNamesByLang(lang), A.map(TaxonVernacularName.getName), (names) => names.sort())

export const getTaxonSingleVernacularNameText = (lang) => A.pipe(getTaxonVernacularNamesText(lang), A.head)
...
export const fetchTaxonomyByName = async (surveyId, name, draft) => {
  ...
  return A.find((taxonomy) => Taxonomy.getName(taxonomy) === name)(taxonomies)
}
...
export const fetchTaxonByCode = async (surveyId, taxonomyUuid, code, draft) => {
  ...
  return A.find((taxon) => Taxon.getCode(taxon) === code)(taxa)
}
...
export const updateTaxonWithNewVernacularNames = async (user, surveyId, taxonomyName, taxonCode, lang, names) => {
  ...
  const vernacularNamesArrayNew = A.map((name) => TaxonVernacularName.newTaxonVernacularName(lang, name))(names)
...
export const importFile = async (user, surveyId, taxonomyName, importFileName) => {
  ...
  const taxonomyExisting = A.find((taxonomy) => Taxonomy.getName(taxonomy) === taxonomyName)(taxonomies)
  let taxonomy = null
  if (taxonomyExisting) {
