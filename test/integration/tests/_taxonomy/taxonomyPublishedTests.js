...
export const taxonPublishedAddVernacularNameTest = async () => {
  ...
  const vernacularNameNew = A.head(vernacularNamesNew)

  // Reload vernacular name
  const taxonReloaded = await TaxonomyUtils.fetchTaxonByCode(surveyId, taxonomyUuid, taxonCode, true)
  const vernacularNameReloaded = A.pipe(Taxon.getVernacularNamesByLang(lang), A.head)(taxonReloaded)
...
export const taxonPublishedUpdateVernacularNamesTest = async () => {
  ...
  const vernacularName1Old = A.head(vernacularNamesOld)
  ...
  const vernacularName1Reloaded = A.find((vn) => TaxonVernacularName.getName(vn) === name1)(vernacularNamesReloaded)
  ...
  const vernacularName2Reloaded = A.find((vn) => TaxonVernacularName.getName(vn) === name2)(vernacularNamesReloaded)
