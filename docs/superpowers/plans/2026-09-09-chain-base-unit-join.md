...
> **Post-execution amendment:** after this plan was fully executed, the user clarified that the sampling-point-data join method requires **both** the base unit's key attribute AND the selected Phase-1 category to be `sampling_point_data` (not the base unit's key alone, as an earlier revision of this doc's summary lines below imply). The predicate's code sample at Task 1 already reflected the two-condition rule; only the prose summary here was stale. The design spec (`docs/superpowers/specs/2026-09-09-chain-base-unit-join-design.md`) is the up-to-date authority — this plan is left as a historical record of the original task breakdown.
