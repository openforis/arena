...
- The category editor (`CategoryDetails`, reachable via "edit" from `FirstPhaseCategorySelector`) is out of scope — categories are survey-wide shared resources, not owned by a single chain, so locking one chain must not restrict editing a category that other chains or node defs may also use. **Clarified post-review:** this means the `CategoryDetails` *page itself* stays fully editable regardless of any chain's lock state (confirmed: `Analysis.js` never wraps it) — it does not mean every navigation entry point into it must stay visible from a locked chain's own UI. The "edit" pencil in `FirstPhaseCategorySelector` is part of this chain's Sampling Design tab and is correctly hidden when locked (`showEdit={editable}`, Task 3); the category remains reachable and editable via the Survey Designer or any other unlocked chain.
