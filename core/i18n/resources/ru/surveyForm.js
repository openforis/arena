...
- **Отключено**: все народные названия таксона группируются и отображаются рядом с таксоном (объединяются в одну запись). В результате список автодополнения содержит меньше элементов.`,
  ...
  cloneFromAnotherSurvey: {
    title: 'Клонировать узел из другой анкеты',
    expandToLoadNodes: 'Разверните, чтобы загрузить узлы',
    loadingSurveyNodes: 'Загрузка узлов анкеты...',
    errorLoadingSurveyNodes: 'Ошибка загрузки узлов анкеты',
    noNodesAvailable: 'Узлы недоступны',
  ...
  printableExport: {
    ...
    includeQrCodeInfo:
      'Добавляет QR-код на первую страницу со ссылкой на доступную только для чтения PDF-копию этой сущности (срок действия — 1 год).',
