...
(это должен быть атрибут типа "код" или "текст"; его значение сопоставляется с дополнительными свойствами, определенными для категории 1-й фазы — имя атрибута не обязательно должно совпадать с именем дополнительного свойства)`,
    ...
    firstPhaseSamplingPointDataJoinMethod: {
      ...
      description:
        'Базовая единица и таблица 1-й фазы соединяются с помощью метода "Данные точек выборки": ключевой атрибут базовой единицы и категория 1-й фазы используют категорию "Данные точек выборки", поэтому отдельный атрибут соединения не требуется.',
    },
