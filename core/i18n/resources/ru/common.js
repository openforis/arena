...
`,
  ...
  nodeDefEdit: {
    ...
    advancedProps: {
      ...
      defaultValuesInfo: `Значения по умолчанию будут присваиваться атрибуту в зависимости от заданных условий/выражений.
    Если указано несколько условий/выражений, будет применено первое, которое удовлетворяет условию "Применять если",
    а результат вычисления его выражения будет присвоен значению атрибута.`,
...
(где "region" - это имя дополнительного свойства, определенного для элемента, а region_attribute_name - это имя атрибута в опросе)`,
      ...
      relevantIfInfo: `По умолчанию узлы всегда актуальны.
    Если условие актуальности не выполняется, узел отображается серым (или скрывается), а любые значения по умолчанию игнорируются.
    Это позволяет условно скрывать разделы формы опроса независимо от ролей пользователей, например показывать поле "Уточните", только если пользователь выбрал "Другое" в предыдущем вопросе.`,
      script: 'Скрипт',
      visibleIf: 'Видимо, если',
      visibleIfInfo: `Если не указано, узел всегда будет видимым.
    Если условие/выражение вычисляется как true, узел будет видимым;
    в противном случае он будет скрыт.
    В отличие от "Скрыто, когда неактуально", скрытому атрибуту будет присвоено значение по умолчанию (если задано).
    Эта функция может использоваться для скрытия целых частей формы опроса на основе определенных условий (например, роли пользователя).`,
    },
    mobileAppProps: {
      hiddenInMobile: {
