...
const contextByDependencyTypeFns = {
  ...
  [Survey.dependencyTypes.applicable]: (survey, nodeDef) =>
...
const expressionsByDependencyTypeFns = {
  ...
  [Survey.dependencyTypes.validations]: R.pipe(
...
const errorKeyByDependencyType = {
  [Survey.dependencyTypes.defaultValues]:
    Validation.messageKeys.nodeDefEdit.defaultValuesInvalid,
  [Survey.dependencyTypes.applicable]:
    Validation.messageKeys.nodeDefEdit.applyIfInvalid,
  [Survey.dependencyTypes.validations]:
    Validation.messageKeys.nodeDefEdit.validationsInvalid,
}
...
const _identifierEval = (survey, nodeDefCurrent, dependencyType) => (
  expr,
  _ctx,
) => {
  const nodeDefContext = contextByDependencyTypeFns[dependencyType](
    survey,
    nodeDefCurrent,
  )
  const selfReferenceAllowed =
    selfReferenceAllowedByDependencyType[dependencyType]
...
const _validateNodeDefExpr = async (survey, nodeDef, dependencyType, expr) => {
  const functions = {
    [Expression.types.Identifier]: _identifierEval(
      survey,
      nodeDef,
      dependencyType,
    ),
...
const _validateExpressionProp = (survey, nodeDef, dependencyType) => async (
  propName,
  item,
) => {
  const expr = R.pathOr(null, propName.split('.'), item)
  return expr
    ? await _validateNodeDefExpr(survey, nodeDef, dependencyType, expr)
    : null
}

const _validateOnlyLastApplyIfEmpty = (nodeDefExpressions, i) => async (
  propName,
  nodeDefExpression,
) => {
  ...
  return R.isEmpty(expr) && i < nodeDefExpressions.length - 1
    ? {
        key:
          Validation.messageKeys.nodeDefEdit
...
const _validateExpressionUniqueness = (nodeDefExpressions, nodeDefExpression) =>
  R.any(
    nodeDefExpr =>
      ...
      NodeDefExpression.getExpression(nodeDefExpr) ===
        NodeDefExpression.getExpression(nodeDefExpression) &&
      NodeDefExpression.getApplyIf(nodeDefExpr) ===
...
const _validateExpression = async (
  ...
  const validateApplyIfUniqueness =
    applyIfUniquenessByDependencyType[dependencyType]

  const validation = await Validator.validate(nodeDefExpression, {
    [NodeDefExpression.keys.expression]: [
      Validator.validateRequired(
        Validation.messageKeys.nodeDefEdit.expressionRequired,
      ),
      ...
      ...(validateApplyIfUniqueness
        ? [
            Validator.validateItemPropUniqueness(
              Validation.messageKeys.nodeDefEdit.applyIfDuplicate,
            )(nodeDefExpressions),
...
export const validate = async (survey, nodeDef, dependencyType) => {
  ...
  const nodeDefExpressions = expressionsByDependencyTypeFns[dependencyType](
    nodeDef,
  )
